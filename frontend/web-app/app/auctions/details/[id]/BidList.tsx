...
export default function BidList({ user, auction }: Props) {
    ...
    useEffect(() => {
        getBidsForAuction(auction.id).then((res: any) => {
            ...
            toast.error(error.message || 'Something went wrong while fetching bids');
    ...
    return (
        <div className='rounded-lg shadow-md'>
            ...
            <div className='overflow-auto h-[350px] flex flex-col-reverse px-2'>
                {bids.length === 0 ? (
                    <EmptyFilter title='No bids for this item yet' subtitle='Please feel free to place a bid!' />
                ) : (
                    <>
                        {bids.map(bid => (
